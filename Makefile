...
.PHONY: build
build: install-tools
	GOOS=$(OS) GOARCH=$(ARCH) go build -o $(BUILD_DIR)/${{ values.binaryName }}
...
dockerbuild:
	...
	docker build . -t ${{ values.destination.owner + "/" + values.destination.repo }}:localdev --build-arg BIN_PATH="./build/${{ values.binaryName }}" --platform linux/$(ARCH)
